	...
	def fetchWarp_BYOSED(self,config):
        ...
        for warp in self.warp_effects:
			...
			if 'SN_FUNCTION' in warp_data:
                ...
                try:
                    sn_param_names,sn_function=_read_ND_grids(os.path.expandvars(os.path.join(self.PATH_VERSION,
			...
			if 'HOST_FUNCTION' in warp_data:
                if 'DIST' in ' '.join([x for x in warp_data.keys() if 'HOST' in x or 'SCALE' in x]):
                    distribution=_get_distribution(warp,{k:warp_data[k] for k in warp_data.keys() if 'HOST' in k or 'SCALE' in k},self.PATH_VERSION,'HOS\
T')
                else:
